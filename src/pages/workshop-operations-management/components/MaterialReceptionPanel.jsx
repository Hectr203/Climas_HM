  ...
  const MaterialReceptionPanel = ({ workOrders = [], onMaterialReception, selectedOrder: propSelectedOrder }) => {
    ...
    const { transferirMaterialesTaller } = useInventarioTaller();
    ...
    // Small parser to extract a numeric value from various quantity fields (handles strings like "10 m")
  ...
  useEffect(() => {
    ...
    const handleOrderSelect = (order) => {
      // Left-side cards are non-interactive per user request — keep this handler inert
      // but preserve implementation for future use if needed.
      console.debug('[MaterialReceptionPanel] local handleOrderSelect called but is disabled');
    };
    ...
    useEffect(() => {
      const loadMaterialsForAllOrders = async () => {
        console.debug('[MaterialReceptionPanel] Loading materials for', workOrders.length, 'orders');
        for (const order of workOrders) {
          const orderKey = order?.ordenTrabajo || order?.id || order?.folio || '';
          if (!orderKey) continue;
          
          // Check if already in cache
          if (orderMaterialsCache[orderKey]) {
            console.debug('[MaterialReceptionPanel] Order', orderKey, 'already in cache, skipping');
            continue;
          }
          
          try {
            console.debug('[MaterialReceptionPanel] Loading materials for order', orderKey);
    ...
    const handleSubmitReception = async () => {
      if (selectedOrder) {
        ...
        // compute total required from reception list (fallback to order field)
        const totalRequiredForSubmit = materialsReception.reduce((s, it) => s + (Number(it.required) || 0), 0) || Number(selectedOrder?.materials?.total || 0);
        const status = receptionData?.received >= (totalRequiredForSubmit || 0) ? 'complete' : 'partial';
        ...
        // Clear form. Parent selection is authoritative (from WorkflowBoard),
        // so we only clear local selection; do not notify parent.
        setSelectedOrder(null);
    ...
    // Helpers to compute materials to render and totals for the selected order
    const selectedOrderKey = selectedOrder ? (selectedOrder?.ordenTrabajo || selectedOrder?.id || selectedOrder?.folio || '') : '';
    const cachedForSelected = selectedOrderKey ? orderMaterialsCache[selectedOrderKey] : null;

    const materialsListForRender = (() => {
      if (Array.isArray(materialsReception) && materialsReception.length) return materialsReception;
      // prefer cached normalized article shape
      if (cachedForSelected && Array.isArray(cachedForSelected.materials) && cachedForSelected.materials.length) {
        return cachedForSelected.materials.map((m, idx) => ({
          key: m.codigo || m.descripcion || `c_${idx}`,
          codigo: m.codigo || null,
          descripcion: m.descripcion || m.nombre || m.producto || m.name || 'Sin descripción',
          required: parseNumeric(m.cantidad ?? m.cantidadConUnidad ?? m.cantidad_total ?? m.cantidadTotal ?? 0) || 0,
          received: parseNumeric(m.received ?? m.recibido ?? 0) || 0,
          missing: Math.max(0, (parseNumeric(m.cantidad ?? m.cantidadConUnidad ?? 0) || 0) - (parseNumeric(m.received ?? m.recibido ?? 0) || 0)),
          raw: m.raw || m
        }));
      }

      if (Array.isArray(approvedMaterials) && approvedMaterials.length) {
        return approvedMaterials.map((m, idx) => ({
          key: m.codigo || m.descripcion || `a_${idx}`,
          codigo: m.codigo || null,
          descripcion: m.descripcion || m.nombre || m.producto || m.name || 'Sin descripción',
          required: Number(m.cantidad ?? m.required ?? 0) || 0,
          received: Number(m.received ?? 0) || 0,
          missing: Math.max(0, (Number(m.cantidad ?? m.required ?? 0) || 0) - (Number(m.received ?? 0) || 0)),
          raw: m.raw || m
        }));
      }

      // try raw shapes on the selectedOrder
      try {
        const rawMaterials = selectedOrder?.materials || selectedOrder?.raw?.materiales || selectedOrder?.raw?.materials || selectedOrder?.materiales || [];
        if (Array.isArray(rawMaterials) && rawMaterials.length) {
          return rawMaterials.map((m, idx) => ({
            key: m.codigo || m.descripcion || m.nombre || `r_${idx}`,
            codigo: m.codigo || null,
            descripcion: m.descripcion || m.nombre || m.name || 'Sin descripción',
            required: Number(m.cantidad ?? m.cantidadConUnidad ?? m.required ?? m.total ?? 0) || 0,
            received: Number(m.received ?? m.recibido ?? 0) || 0,
            missing: Math.max(0, (Number(m.cantidad ?? m.required ?? m.total ?? 0) || 0) - (Number(m.received ?? m.recibido ?? 0) || 0)),
            raw: m
          }));
        }
      } catch (e) {
    ...
    const totalsForSelected = (() => {
      const list = materialsListForRender || [];
      const totalRequired = list.reduce((s, it) => s + (Number(it.required) || 0), 0);
      const totalReceived = list.reduce((s, it) => s + (Number(it.received) || 0), 0);
      const totalPending = Math.max(0, totalRequired - totalReceived);
      return { totalRequired, totalReceived, totalPending };
    })();

    // Lightweight helper to compute totals for an arbitrary order (used in orders list preview)
    const computeTotalsForOrder = (order, cachedMaterials, existingReception) => {
      const orderKey = order?.ordenTrabajo || order?.id || order?.folio || '';
      
      try {
        // Priority 1: Check if order has recepcionMateriales from database
        const dbReception = order?.recepcionMateriales;
        if (dbReception && Array.isArray(dbReception.materials) && dbReception.materials.length) {
          const totalRequired = dbReception.materials.reduce((s, m) => s + (parseNumeric(m.required ?? m.cantidad ?? 0) || 0), 0);
          const totalReceived = dbReception.materials.reduce((s, m) => s + (parseNumeric(m.received ?? m.recibido ?? 0) || 0), 0);
          const totalPending = parseNumeric(dbReception.pendientes ?? dbReception.totalPendientes ?? 0) || Math.max(0, totalRequired - totalReceived);
          console.log(`📋 ${orderKey}: Usando recepcionMateriales de BD - Req:${totalRequired} Rec:${totalReceived} Pend:${totalPending}`);
          return { totalRequired, totalReceived, totalPending };
        }
        
        // Priority 2: If cache exists, use it
        if (cachedMaterials && Array.isArray(cachedMaterials.materials) && cachedMaterials.materials.length) {
          const totalRequired = cachedMaterials.materials.reduce((s, m) => s + (parseNumeric(m.cantidad ?? m.cantidadConUnidad ?? m.cantidad_total ?? m.cantidadTotal ?? m.cantidad ?? m.required ?? 0) || 0), 0);
          const totalReceived = (existingReception && Array.isArray(existingReception.materials) && existingReception.materials.length)
            ? existingReception.materials.reduce((s, m) => s + (parseNumeric(m.received ?? m.recibido ?? m.have ?? 0) || 0), 0)
            : (parseNumeric(existingReception?.cantidadRecibida ?? existingReception?.received ?? 0));
          console.log(`✓ ${orderKey}: Usando caché - Req:${totalRequired} Rec:${totalReceived}`);
          return { totalRequired, totalReceived, totalPending: Math.max(0, totalRequired - totalReceived) };
        }

        // Priority 3: Check localStorage reception
        if (existingReception && Array.isArray(existingReception.materials) && existingReception.materials.length) {
          const totalRequired = existingReception.materials.reduce((s, m) => s + (parseNumeric(m.required ?? m.cantidad ?? 0) || 0), 0);
          const totalReceived = existingReception.materials.reduce((s, m) => s + (parseNumeric(m.received ?? m.recibido ?? 0) || 0), 0);
          console.log(`💾 ${orderKey}: Usando localStorage - Req:${totalRequired} Rec:${totalReceived}`);
          return { totalRequired, totalReceived, totalPending: Math.max(0, totalRequired - totalReceived) };
        }

        // Priority 4: If order contains raw materials array, sum those
        const rawMaterials = order?.raw?.materiales || order?.raw?.materials || order?.materiales || order?.materials;
        if (Array.isArray(rawMaterials) && rawMaterials.length) {
          const totalRequired = rawMaterials.reduce((s, m) => s + (parseNumeric(m.cantidad ?? m.cantidadConUnidad ?? m.cantidad_total ?? m.cantidadTotal ?? m.cantidad ?? m.required ?? m.qty ?? 0) || 0), 0);
          const totalReceived = rawMaterials.reduce((s, m) => s + (parseNumeric(m.received ?? m.recibido ?? m.have ?? 0) || 0), 0) || (parseNumeric(order?.materials?.received ?? 0));
          console.log(`→ ${orderKey}: Usando raw materials (${rawMaterials.length}) - Req:${totalRequired} Rec:${totalReceived}`);
          return { totalRequired, totalReceived, totalPending: Math.max(0, totalRequired - totalReceived) };
        }

        // Priority 5: Fallback to order.materials totals if present
        if (order?.materials && (order.materials.total || order.materials.received)) {
          const totalRequired = parseNumeric(order.materials.total ?? 0) || 0;
          const totalReceived = parseNumeric(order.materials.received ?? 0) || 0;
          console.log(`⚠ ${orderKey}: Usando order.materials - Req:${totalRequired} Rec:${totalReceived}`);
          return { totalRequired, totalReceived, totalPending: Math.max(0, totalRequired - totalReceived) };
        }

        console.log(`✗ ${orderKey}: SIN MATERIALES`);
        return { totalRequired: 0, totalReceived: 0, totalPending: 0 };
      } catch (e) {
        console.error(`ERROR ${orderKey}:`, e);
        return { totalRequired: 0, totalReceived: 0, totalPending: 0 };
      }
    };

    // Ensure materialsReception is initialized from cached/approved materials when possible
    useEffect(() => {
      if (!selectedOrder) return;
      try {
        const hasReception = Array.isArray(materialsReception) && materialsReception.length > 0;
        const hasCached = cachedForSelected && Array.isArray(cachedForSelected.materials) && cachedForSelected.materials.length > 0;
        const hasApproved = Array.isArray(approvedMaterials) && approvedMaterials.length > 0;
        if (!hasReception && (hasCached || hasApproved)) {
          const toInit = hasCached ? cachedForSelected.materials : approvedMaterials;
          const persisted = loadReceptionFromTaller(selectedOrderKey);
          setMaterialsReception(initMaterialsReception(toInit, persisted));
        }
      } catch (e) {}
    }, [selectedOrderKey, cachedForSelected, approvedMaterials]);

    // Log importante: ver qué órdenes llegan y qué datos tienen
    console.log('===== MATERIAL RECEPTION PANEL =====');
    console.log('Total workOrders:', workOrders?.length);
    console.log('workOrders completo:', workOrders);
    console.log('orderMaterialsCache:', orderMaterialsCache);
    console.log('propSelectedOrder:', propSelectedOrder);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            <h3 className="font-medium mb-3">
              {propSelectedOrder ? 'Orden Seleccionada' : 'Órdenes Pendientes'}
            </h3>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {/* If there's a selected order, show ONLY that order */}
              {propSelectedOrder ? (
                <div className="mb-2">
                  {(() => {
                    const order = propSelectedOrder;
                    const orderKey = order?.ordenTrabajo || order?.id || order?.folio || '';
                    ...
                    return (
                      <div
                        ...
                        className="p-3 border-2 border-primary bg-primary/10 rounded-lg"
                      >
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {cachedMaterials && cachedMaterials.materials && cachedMaterials.materials.length > 0 && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-green-100 text-green-700 border border-green-200">
                                  ✓ {cachedMaterials.materials.length}
                                </span>
                              )}
                            </div>
                            <div className="text-sm text-muted-foreground">{order?.clientName}</div>
                            {(() => {
                              const totals = computeTotalsForOrder(order, cachedMaterials, existingReception);
                ...
                // If no selected order, show all workOrders
                workOrders?.map((order) => {
                const orderKey = order?.ordenTrabajo || order?.id || order?.folio || '';
                const cachedMaterials = orderMaterialsCache[orderKey];
                const existingReception = loadReceptionFromTaller(orderKey);
                
                // Log para cada orden individual
                console.log(`Orden ${orderKey}:`, {
                  order,
                  hasCachedMaterials: !!cachedMaterials?.materials?.length,
                  cachedMaterials,
                  hasReception: !!existingReception
                });
                
                return (
                  <div
                    key={order?.ordenTrabajo || order?.id}
                    className={`p-3 border rounded-lg transition-colors ${(propSelectedOrder?.ordenTrabajo || propSelectedOrder?.id) === (order?.ordenTrabajo || order?.id) ? 'border-primary bg-primary/10' : 'border-muted'}`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {/* Compact materials list for orders list: try cache, saved reception, or raw fields */}
        {(() => {
          // Debug: log what data is available for this order
          console.debug('[MaterialReceptionPanel] Preview for order', orderKey, {
            hasCached: !!(cachedMaterials && Array.isArray(cachedMaterials.materials) && cachedMaterials.materials.length),
            hasReception: !!(existingReception && Array.isArray(existingReception.materials) && existingReception.materials.length),
            hasRawMateriales: !!(order?.raw?.materiales && Array.isArray(order.raw.materiales) && order.raw.materiales.length),
            hasMateriales: !!(order?.materiales && Array.isArray(order.materiales) && order.materiales.length),
            order: order
          });
          
          const materialsPreview = (cachedMaterials && Array.isArray(cachedMaterials.materials) && cachedMaterials.materials.length)
            ? cachedMaterials.materials.slice(0, 2)
            : (existingReception && Array.isArray(existingReception.materials) && existingReception.materials.length)
              ? existingReception.materials.slice(0, 2)
              : (Array.isArray(order?.raw?.materiales) && order.raw.materiales.length) ? normalizeAndAggregate(order.raw.materiales).slice(0, 2)
              : (Array.isArray(order?.materiales) && order.materiales.length) ? normalizeAndAggregate(order.materiales).slice(0, 2)
              : null;

          if (!materialsPreview || !materialsPreview.length) {
            console.debug('[MaterialReceptionPanel] No preview materials for', orderKey);
            return null;
          }                        return (
                            <div className="mt-2 space-y-1 text-xs">
                              {materialsPreview.map((mat, i) => (
                                <div key={i} className="bg-white border rounded px-2 py-1">
                                  <div className="font-medium truncate">{mat.descripcion || mat.nombre || mat.producto || mat.name || 'Sin descripción'}</div>
                                  <div className="text-muted-foreground text-[11px] mt-1">
                                    Req: {parseNumeric(mat.cantidad ?? mat.cantidadConUnidad ?? mat.cantidad_total ?? mat.cantidadTotal ?? mat.required ?? 0) || 0} • Rec: {parseNumeric(mat.received ?? mat.recibido ?? mat.have ?? 0) || 0}
                                  </div>
                                </div>
                              ))}
                            </div>
            ...
            {workOrders?.length === 0 && !selectedOrder && !propSelectedOrder && (
          ...
          <div>
            {selectedOrder ? (
              <div className="space-y-4">
                <div className="bg-muted p-4 rounded-lg">
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <div>
                      ...
                      {typeof totalsForSelected?.totalRequired === 'number' && totalsForSelected.totalRequired > 0 ? (
                        <span> {totalsForSelected.totalRequired} items</span>
                      ) : (
                        <span> —</span>
                      )}
                    </div>
                ...
                {materialsListForRender && materialsListForRender.length > 0 && (
                  <div className="p-3 border rounded-lg bg-white">
                    ...
                    <div className="text-sm space-y-3 max-h-64 overflow-y-auto">
                      {materialsListForRender.map((m, idx) => (
                ...
                {/* Status Alert */}
                {receptionData?.received < (totalsForSelected?.totalRequired || 0) && (
